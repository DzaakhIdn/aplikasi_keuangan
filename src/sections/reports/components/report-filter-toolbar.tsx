...
export function ReportFilterToolbar({
  ...
  const canReset = Object.values(filters).some((value) => value && value !== "all");
    ...
    <Card sx={{ p: 2.5, mb: 3 }}>
      <Box
        sx={{
          display: "grid",
          gap: 2,
          gridTemplateColumns: { xs: "1fr", sm: "1fr 1fr", xl: "1fr 1fr 180px 180px auto" },
          alignItems: "center",
        }}
      >
        <TextField
        ...
        <Box sx={{ display: "flex", gap: 1, flexWrap: "wrap", justifyContent: { xl: "flex-end" } }}>
          {canReset && (
            <Button color="error" onClick={onReset} startIcon={<Iconify icon="solar:restart-bold" />}>
              Reset
            </Button>
          )}
          <Button variant="outlined" onClick={onExport} startIcon={<Iconify icon="solar:export-bold" />}>
            Excel
          </Button>
          <Button variant="contained" onClick={onPrint} startIcon={<Iconify icon="solar:printer-minimalistic-bold" />}>
            Cetak / PDF
          </Button>
        </Box>
      </Box>

...
export function PaymentBreakdownCard({ rows, loading }: { rows: PaymentBreakdown[]; loading: boolean }) {
  return (
    <Card sx={{ minWidth: 0, height: "100%" }}>
      <CardHeader
        title="Pembayaran Berdasarkan Jenis"
        subheader="Perbandingan tagihan dan uang yang diterima"
        avatar={<ReportSectionIcon icon="solar:pie-chart-2-bold-duotone" color="info.main" />}
      />
            ...
            <Box key={row.name}>
              <Stack direction="row" spacing={2} sx={{ justifyContent: "space-between", mb: 0.75 }}>
                ...
                <Box sx={{ textAlign: "right", flexShrink: 0 }}>
                  <Typography variant="subtitle2">{fCurrency(row.paid)}</Typography>
              ...
              <Box sx={{ height: 8, overflow: "hidden", borderRadius: 999, bgcolor: "background.neutral" }}>
                <Box sx={{ width: `${percentage}%`, height: 1, borderRadius: "inherit", bgcolor: "success.main" }} />
              </Box>
            </Box>
          );
        })}

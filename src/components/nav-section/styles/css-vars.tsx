...
import type { NavCssVars } from '../types';

// ----------------------------------------------------------------------

export const bulletColor = { dark: '#282F37', light: '#EDEFF2' };

function colorVars(theme: Theme, variant: 'vertical' | 'mini' | 'horizontal'): NavCssVars {
  const {
    vars: { palette },
  } = theme;
...
function verticalVars(theme: Theme): NavCssVars {
  const { shape } = theme;
...
function miniVars(theme: Theme): NavCssVars {
  const { shape } = theme;
...
function horizontalVars(theme: Theme): NavCssVars {
  const { shape } = theme;
